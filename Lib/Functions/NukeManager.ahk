...
CheckForWave50() {
    if (ok := FindText(&X, &Y, 245, 102, 287, 125, 0.10, 0.10, Wave50)) {
        return true
    }
    return false
}
...
CheckForWave20() {
    if (ok := FindText(&X, &Y, 245, 102, 287, 125, 0.10, 0.10, Wave20)) {
        return true
    }
    return false
}
...
CheckForWave15() {
    return FindText(&X, &Y, 245, 102, 287, 125, 0.10, 0.10, Wave15)
}
...
HandleSpecificWaveNuke(nukeDelay) {
    ...
    switch wave {
        ...
        case "15":
            if (!CheckForWave15())
                return false
            AddToLog("Found Wave 15, preparing to nuke after " nukeDelay / 1000 " seconds...")
            return true
