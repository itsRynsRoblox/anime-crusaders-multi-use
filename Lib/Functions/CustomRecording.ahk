...
StartRecordingActionsForMap(*) {
    ...
    for btn in ["LButton", "RButton", "MButton"] {
        activeActionHotkeys.Push(Hotkey("~*" btn, RecordMouseDown.Bind(btn)))
        activeActionHotkeys.Push(Hotkey("~*" btn " up", RecordMouseUp.Bind(btn)))
    }

    for key in GetValidKeyList() { ; your list of keys
        activeActionHotkeys.Push(Hotkey("~*" key, RecordKeyDownAction.Bind(key)))
        activeActionHotkeys.Push(Hotkey("~*" key " up", RecordKeyUpAction.Bind(key)))
    }

    ; Stop hotkey
    activeActionHotkeys.Push(Hotkey("~LShift", StopRecordingActions))
...
RemoveActiveHotkeys() {
    ...
    for hk in activeActionHotkeys {
        ; Only delete if hk is a Hotkey object
        if IsObject(hk)
            try hk.Delete()
    }
